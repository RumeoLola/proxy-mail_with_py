...
# Meaning function
def email_sender():
    global email_address, email_password, receiver_email
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=ssl_context) as server:  # using port 465
            server.login(email_address, email_password)
            message = Message(From=email_address,
                              To=receiver_email,
                              charset="utf-8")
            message.Subject = "My email to you"
            message.Body = text
